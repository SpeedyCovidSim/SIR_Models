...
def gillespieFirstNetwork(tMax, network, iTotal, sTotal, rTotal, numSusNei, rates, susceptible, alpha, beta, tInit = 0.0):
    '''
    ...
    while t[-1] < tMax and iTotal != 0:
        ...
        deltaTs = -np.log(1-random.uniform(size=len(rates)))/rates
        deltaT = np.min(deltaTs)
        eventIndex = np.argmin(deltaTs)
