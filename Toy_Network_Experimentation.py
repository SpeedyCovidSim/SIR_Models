...
def main(single_lattices=True, multi_lattices=True,k_random=True,k_workplace=True,strong_single_lattices=True,k_erdos_neighbourhood=True,strong_single_prop=True,multi_lattices_prop=True,single_lattices_prop=True):
    '''
    ...
    if(strong_single_prop):
        ...
        fig = plt.figure()
        plt.plot(t, I2, color="blue",label="N = 8",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I4, color="green",label="N = 16",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I8, color="red",label="N = 32",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I16, color="black",label="N = 64",lw = 2, alpha=0.5,figure=fig)
        plt.legend()
        plt.xlabel("Time")
    ...
    if(multi_lattices_prop):
        ...
        fig = plt.figure()
        plt.plot(t, I8, color="blue",label="N = 8",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I16, color="green",label="N = 16",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I32, color="red",label="N = 32",lw = 2, alpha=0.5,figure=fig)
        plt.plot(t, I64, color="black",label="N = 64",lw = 2, alpha=0.5,figure=fig)
        plt.legend()
        plt.xlabel("Time")
        plt.ylabel("Number of Infected Individuals")
        plt.title(f"SIR model of a varying number of multi-seeded households")
    ...
    if(single_lattices_prop):
        ...
        for inf in infecteds:
            ...
            rates[inf] = maxbeta*numSusNei[inf]/network.degree(inf)
        title=f"SIR model with {N} households of {n} people, \n 1 link between adjacent households"
        fname=f"PythonPlotting/Single_Household_Prop/{N}_Households"
        S2, I2, R2 = general_proportional_SIR_simulation(j, gillespieDirectNetwork, tMax, network, iTotal, sTotal, rTotal, numSusNei, rates, susceptible, 
        maxalpha, maxbeta, title, fname)
        ...
        plt.title(f"SIR model of a varying number of single seeded households")
        plt.savefig(f"PythonPlotting/Single_Household_Prop/N_Comparison")
        plt.savefig(f"PythonPlotting/Comparisons/Single_N_Comparison_prop")

        

if __name__=="__main__":
    main(single_lattices=False, multi_lattices=False,k_random=False,k_workplace=False,strong_single_lattices=False,k_erdos_neighbourhood=False,strong_single_prop=False,multi_lattices_prop=False,single_lattices_prop=True)
